...
$nombre = $direccion = $documento = "";
$nombre_err = $direccion_err = $documento_err = "";
...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
    ...
    if (empty($input_documento)) {
        $documento_err = "Porfavor ingresa nro de documento";
    ...
    if (empty($nombre_err) && empty($direccion_err) && empty($documento_err)) {
        // Prepare an update statement
        $sql = "UPDATE EMPLEADOS SET nombre=?, direccion=?, nrodocumento=? WHERE id=?";
        ...
        if ($stmt = $link->prepare($sql)) {
            ...
            $stmt->bindParam(3, $param_documento, PDO::PARAM_STR);
            $stmt->bindParam(4, $param_id, PDO::PARAM_STR);
